...
fb-img: https://i.imgur.com/UlstyySt.png
share-img: https://i.imgur.com/UlstyySt.png
...
focused on beginner R users, **BeginneRs**. We’ve had several special
...
all having different OSs, versions of packages, versions of R itself
...
integration with R Studio) and the use of Docker. Put very simply you
...
of the participants to run R inside a container, an enclosed environment
...
author the mini-books, “Create a Word document with R Markdown” and
...
output such as Word document, PowerPoint presentation, html,
...
-   Code chunk and options: Where all your code (can be languages
    besides R) that you want to be run should be. Chunk options such as
...
If `@GotaMorishita` takes out 90% of the houses/apartments from the
training data and keep those as candidates for the test data, it means
...
set wouldn’t be as accurate as keeping that data in the training set.
This problem, called co-variate shifting, is when the training data and
...
-   [hana\_orin](https://twitter.com/hana_orin): [Using Hierarchical
    Linear Modeling to predict housing prices]()
...
Tokyo.R happens almost monthly and it’s a great way to mingle with
